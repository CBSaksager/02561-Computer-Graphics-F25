...
async function main() {
  ...
  canvas.addEventListener("click", function (ev) {
    ...
    if (drawMode == 0) {
      ...
      let colorArray = [];
      colorArray.push(
        ...Array(verts_per_point).fill(
          colors[colorMenu.selectedIndex]
        )
      );
      ...
      // Triangle mode
      // Store point pos and color
      trianglePoints.push(mousepos);
      triangleColors.push(colors[colorMenu.selectedIndex]);
      if (trianglePoints.length > 3) {
        // Should not happen - reset just in case
        trianglePoints = [];
        triangleColors = [];
      } else if (trianglePoints.length == 3) {
        // We have 3 points - draw the triangle
        positions.push(...trianglePoints);
        device.queue.writeBuffer(
          positionBuffer,
          index * sizeof["vec2"],
          flatten(positions)
        ...
        let colorArray = [];
        colorArray.push(...triangleColors);
        device.queue.writeBuffer(
          colorBuffer,
          index * sizeof["vec4"],
          flatten(colorArray)
      ...
      if (circlePoints.length > 2) {
        ...
        // We have center and a point on the perimeter - draw the circle
        const center = circlePoints[0];
        const centerColor = circleColors[0];
        const edge = circlePoints[1];
        const edgeColor = circleColors[1];
        const radius = Math.sqrt(
          (edge[0] - center[0]) * (edge[0] - center[0]) +
            (edge[1] - center[1]) * (edge[1] - center[1])
        );
        ...
        for (let i = 0; i <= num_points; i++) {
          ...
          const x1 = center[0] + radius * Math.cos(theta);
          const y1 = center[1] + radius * Math.sin(theta);
          perimitor.push(vec2(x1, y1));
        }

        for (let i = 0; i < num_points; i++) {
          positions.push(center, perimitor[i], perimitor[i + 1]);
          colorArray.push(centerColor, edgeColor, edgeColor);
        }
  ...
  var drawModeMenu = document.getElementById("drawModeMenu");
  drawModeMenu.addEventListener("change", function () {
    drawMode = parseInt(drawModeMenu.value);
    trianglePoints = []; // Clear any stored triangle points
    triangleColors = [];
    circlePoints = []; // Clear any stored circle points
    circleColors = [];
